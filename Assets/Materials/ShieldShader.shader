...
//
            ...
            // }
            //float sceneZ = LinearEyeDepth(tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(i.screenuv)));
            //float sceneZ = LinearEyeDepth (SAMPLE_DEPTH_TEXTURE_PROJ(_CameraDepthTexture, UNITY_PROJ_COORD(i.screenuv).r));
            //loat partZ = i.eyeDepth;
            //float sceneZ = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, i.uv);
            //float partZ = i.depth;
            //float fade = saturate ((sceneZ-partZ) / 10);
            //i.color.a *= fade;

             float rim = 1 - abs(dot(i.normal, normalize(i.viewDir))) * 4;
             float glow = saturate(rim);

                fixed4 sweetColor = fixed4(lerp(_Color.rgb, fixed3(1,1,1), pow(glow, _Edge)), 1);

                fixed4 hexes = texColor(i, rim) / 10;
